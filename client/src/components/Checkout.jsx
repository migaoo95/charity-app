...
function Checkout({disabled, items, shipping}) {
  ...
  const [shippingData, setShippingData] = useState([{typeName: 'Standard', cost: 5, duration: [3,5]},{typeName: 'Express', cost: 9, duration: [1,3]}])
  useEffect(() => {
...
if(items){
  ...
  return (
    <div className={classes.buttonContainer}>
      <button disabled={disabled}  className={ `${disabled && classes.buttonContainer__disabled}`} onClick={handleCheckout}>
        Checkouttt
      </button>
    </div>
  );
